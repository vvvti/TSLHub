...
export class Vehicules extends Component {
  ...
  TableHead(props){
    ...
    <th scope="col" key={heder}>{heder}</th>);
  ...
  TableBody(props){
    ...
    <tr>
      {element.map((item)=><td scope="col">{item}</td>)}
    </tr>
    );
    return (
  ...
  componentDidMount() {
    ...
    const tableBody =  [["Mark1","Mark2","Mark3","Mark4"],
                        ["Mark5","Mark2","Mark3","Mark4"],
                        ["Mark5","1998","Panda","Fiat"]];

    this.setState({HederList: colmnsNames,
                   BodyList: tableBody,
  ...
  filterTable(e){
    ...
    var filteredTable = table.filter(column=>column[this.state.FilterIndex].toLowerCase().includes(text.toLowerCase()));
  ...
  TableFilter(props){
    ...
    return( 
      ...
      <form className="form-inline float-left" >
        ...
        <input className = "form-control mb-2 mr-sm-2" onInput={this.filterTable.bind(this)}/>
          <select className = "form-control mb-2 mr-sm-2" onChange={this.handleSelectChange}>
            {listItems}
          </select>
      </form>
      <Link to="/addvehicule" className="btn btn-primary btn-lg active float-right" >Dodaj Pojazd +</Link>
